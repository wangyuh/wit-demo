...
var ThingView = (function () {
    ...
    var s_fileversion = "0.70.2.0";
    var s_productversion = "0.70.2+O5CGjk";
    var s_productname = "ThingView 0.70";
    var doCapture = false;
    var captureWrapper;
    var requestID = null;
    ...
    var returnObj = {
        ...
        /**
          ...
          * via the following API so only deltas from the defaults are required  {@link ThingView.SetDefaultSystemPreferences}
          * @memberof ThingView
        ...
        /**
          ...
          * It is recommended to use the {@link ThingView.CreateCVApplication|CreateCVApplication} API to create
        ...
        DestroyApplication: function(app) {
            _destroyApplication(app);
        },
        DestroyAllApplications: function() {
            _destroyAllApplications();
        },
    ...
    function _ClearCanvas() {
        if (_IsPDFSession()) {
            var canvas = _getCanvas(_currentSession);
    ...
    function _deleteSession(session) {
        if (_currentSession == session) {
            _currentSession.delete();
            _currentSession = null;
            _currentApp = null;
        }
        var canvas = _getCanvas(session);
        if (canvas != null && canvas.parentElement != null) {
            canvas.parentElement.removeChild(canvas);
        }
    }
    
    function _destroyApplication(app) {
        if (_currentApp == app) {
            _currentSession.delete();
            _currentSession = null;
            _currentApp = null;
    ...
    function _destroyAllApplications() {
        if (_currentApp) {
            _currentSession.delete();
            _currentApp.delete();
            _currentSession = null;
        ...
        let apps = Module.Application.GetAllApplications();
        ...
        Module.Application.DestroyAllApplications();
    }                
    ...
    function _getCanvas(session) {
        if(!session) return null;
        var view_html = session.GetActiveView();
        if (!view_html) return null;
        var canvasId = view_html.GetCanvasName();
	view_html.delete();
